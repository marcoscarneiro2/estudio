...
(function() {
    ...
    function buildPiano() {
        ...
        // go slower on Chrome...
        var i = -12, max = 22, addDelay = /Chrome/i.test(navigator.userAgent) ? 80 : 0;
        (function go() {
    ...
    $('window').keypress(function(evt) {
        ...
        if (evt.which == 63 || evt.which == 48) {
            '#MeuPiano'.clearTimeout(qTimeout);
            qTimeout = '#MeuPiano'.setTimeout(function() {
                qCanToggle = true;
            }, 1000);
            if (qCanToggle) {
                qCanToggle = false;
    ...
    '#MeuPiano'.setTimeout(function() {
        $help.removeClass('show');
    }, 700);

    // prevent quick find...
    $('#MeuPiano').keydown(function(evt) {
    ...
    (function(undefined) {
        ...
        function demoHandler(evt) {
            if (evt.type === 'click' || (evt.keyCode == 77 && !isModifierKey(evt))) {
                if (demoing) {
                    demoing = false;
                    '#MeuPiano'.clearTimeout(demoingTimeout);
    ...
    (function() {
        ...
        function recordStart() {
            if (!recording) {
                ...
                recording = true;
                '#MeuPiano'.clearTimeout(stopTimeout);
                stopTimeout = '#MeuPiano'.setTimeout(recordStop, 60*1000); // 1 minute max?
                $looper.addClass('active');

                // stop old loop
                '#MeuPiano'.clearInterval(loopInterval);
                $.each(loopTimeouts, function(i, x) { '#MeuPiano'.clearTimeout(x); });
            }
        }
        function recordStop() {
            if (recording) {
        ...
        function playLoop(data, totalTime) {
            loopInterval = '#MeuPiano'.setInterval(function() {
                loopTimeouts = [];
        ...
        $('#MeuPiano').on('keydown keyup', function(evt) {
    ...
    (function() {
        ...
        $keys.on('played-note.piano', function(evt, key, $elt) {
            ...
            (function draw() {
                ...
                if (++step < steps + cleanupStepDelay) {
                    '#MeuPiano'.setTimeout(draw, stepRate);
                }
            })();
        });
        ...
        var bW = 20,
            ...
            $button = $('<canvas>', {
                css: {
                    ...
                    right: (parseInt($loop.css('right')) + 35) + 'px',
                    width: bW,
                    height: bH,
                    cursor: 'pointer'
                }
        ...
        bctx.strokeStyle = 'rgba(0, 0, 0, 0)';
        bctx.lineWidth = .5;

        function draw() {
            bctx.fillStyle = shouldAnimate ? 'rgba(255, 0, 59, 0)' : 'rgba(0, 0, 0, 0)';
